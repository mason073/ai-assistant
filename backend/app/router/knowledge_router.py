...
async def process_document(
    document_id: str, file_path: str, kb_name: str, db_session_factory
):
...
async def create_knowledge_base(
    ...
    existing = (
        db.query(KnowledgeBase)
        .filter(
            KnowledgeBase.user_id == current_user.id, KnowledgeBase.name == kb_data.name
        )
        .first()
    )
    ...
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="已存在同名知识库"
        )
...
async def get_knowledge_base(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="无效的知识库ID格式"
        )
...
async def upload_document(
    ...
    background_tasks.add_task(
        process_document, str(doc.id), file_path, kb.name, SessionLocal
    )
...
async def get_document_chunks(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="无效的ID格式"
        )
    ...
    if doc.status != "completed":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="文档尚未处理完成"
        )

    # 从 Milvus 获取切片
    collection_name = f"kb_{kb.name}".lower().replace(" ", "_")
    print(
        f"[get_document_chunks] 查询切片: collection={collection_name}, filename={doc.filename}"
    )

...
# 支持的文件类型
SUPPORTED_FILE_TYPES = {".pdf", ".docx", ".xlsx", ".xls", ".txt"}
...
async def upload_document(
    ...
    index_name: str = Query(
        default="policy_documents", description="Index name for storing the document"
    ),
):
    """
...
async def get_document_list(
    ...
    keywords: Optional[str] = Query(
        None, description="Keywords to filter document titles"
    ),
    document_id: Optional[str] = Query(
        None, description="Filter by specific document ID"
    ),
    ...
    try:
        ...
        error_msg = str(e)
        if (
            "document None" in error_msg
            or "You don't own the document None" in error_msg
        ):
            return {"code": 0, "data": {"docs": [], "total": 0}}
...
async def delete_documents(
    request: DeleteDocumentsRequest, service_info: tuple = Depends(get_document_service)
):
    """
    ...
    try:
        ...
        response = doc_service.delete_documents(
            dataset_id=dataset_id, document_ids=request.document_ids
        )

...
from models.chat import ChatMessage, ChatSession, LongTermMemory
...
async def get_memory(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="无效的记忆ID格式"
        )

    memory = (
        db.query(LongTermMemory)
        .filter(
            LongTermMemory.id == mem_uuid, LongTermMemory.user_id == current_user.id
        )
        .first()
    )
...
async def create_memory_from_session(
    ...
    if not messages:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="会话没有消息"
        )
...
async def delete_memory(
    ...
    success = memory_service.delete_memory(
        db=db, memory_id=memory_id, user_id=str(current_user.id)
    )

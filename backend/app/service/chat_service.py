...
class ChatService:
    ...
    def retrieve_from_knowledge_base(
        self, question: str, dataset_id: str
    ) -> List[Dict[str, Any]]:
        """
    ...
    def retrieve_from_web(self, question: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 提取并格式化搜索结果
            formatted_results = self.web_search_service.extract_search_results(
                search_results
            )
        ...
        try:
            ...
            for i, score in enumerate(similarity_scores):
                if i < len(documents):  # 防止索引越界
                    ...
                    documents[i]["content_with_weight"] = (
                        f"{documents[i]['content']} (相关度: {float(score):.2f})"
                    )

            # 根据新权重排序（权重越高越相关）
            sorted_docs = sorted(
                documents, key=lambda x: x.get("weight", 0), reverse=True
            )
            ...
            for doc in sorted_docs:
                ...
                if total_tokens + doc_tokens > self.max_tokens:
                    print(
                        f"跳过文档: {doc.get('title', '无标题')} ({doc_tokens} tokens)，会超出限制"
                    )
                    continue
                ...
                print(
                    f"添加文档: {doc.get('title', '无标题')} ({doc_tokens} tokens), 累计: {total_tokens}/{self.max_tokens}"
                )
        ...
        if not retrieved_content:
            ...
            for i, ref in enumerate(retrieved_content):
                formatted_refs.append(
                    f"[{i + 1}] [{ref['source']}] {ref['content_with_weight']}"
                )
                ...
                if chunk.choices[0].finish_reason == "stop":
                    ...
                    if session_id and model_answer:
                        self.session_service.add_message(
                            session_id, "assistant", model_answer
                        )

...
class ChartGenerator:
    ...
    def generate(
        self, data: Union[Dict, List], chart_type: str, title: str, **kwargs
    ) -> Dict[str, Any]:
        """
        生成图表配置
    ...
    def generate_table(
        self, data: Union[Dict, List], title: str, **kwargs
    ) -> Dict[str, Any]:
        """
        生成表格配置
    ...
    def _parse_series_data(self, data: Union[Dict, List]) -> tuple:
        ...
        if isinstance(data, dict):
            ...
            if data and isinstance(data[0], dict):
                ...
                series_data = [
                    {"name": "数值", "data": [item.get("value", 0) for item in data]}
                ]
            else:
                # 格式: [value1, value2, ...]
    ...
    def _parse_pie_data(self, data: Union[Dict, List]) -> List[Dict]:
        ...
        if isinstance(data, dict):
            ...
            if data and isinstance(data[0], dict):
                ...
                # 格式: [value1, value2, ...]
                pie_data = [
                    {"name": f"项目{i + 1}", "value": v} for i, v in enumerate(data)
                ]

...
class SmartDataAnalyzer:
    ...
    def analyze(
        self, data: Union[List[Dict], List[str], Dict], analysis_type: str = "auto"
    ) -> Dict[str, Any]:
        """
        执行数据分析
    ...
    def _detect_analysis_type(
        self, profile: List[ColumnProfile], data: List[Dict]
    ) -> str:
        """自动检测最佳分析类型"""
    ...
    def _analyze_trend(
        self, data: List[Dict], profile: List[ColumnProfile]
    ) -> Dict[str, Any]:
        """趋势分析"""
        insights = []
        statistics = {}
        ...
        if time_col and value_col:
            ...
            values = [
                ...
                if self._is_numeric(row.get(value_col))
            ]
        ...
        if cat_col and value_col:
            ...
            if category_values:
                ...
                sorted_by_sum = sorted(
                    category_stats.items(), key=lambda x: x[1]["sum"], reverse=True
                )
                if sorted_by_sum:
                    ...
                    if len(sorted_by_sum) > 1:
                        ...
                        insights.append(
                            f"最低: {bottom_cat[0]} ({bottom_cat[1]['sum']:.2f})"
                        )
    ...
    def _analyze_general(
        ...
        for p in profile:
            if p.statistics:
                insights.append(
                    f"{p.name}: 范围 {p.statistics.get('min', 'N/A')} - {p.statistics.get('max', 'N/A')}"
                )

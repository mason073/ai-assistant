...
SEARCH_API_KEY = os.getenv(
    "BOCHA_API_KEY", "Bearer sk-392ef5953eaa4c43be43e6daab4e82a4"
)
...
class ResearchService:
    ...
    async def _research_with_react(
        ...
        try:
            ...
            if memory:
                ...
                yield serialize_event(
                    {"type": "error", "content": "未能收集到足够的研究信息"}
                )

        except Exception as e:
            logging.error(f"ReAct research error: {e}")
            yield serialize_event(
                {"type": "error", "content": f"研究过程出错: {str(e)}"}
            )
    ...
    async def _research_classic(
        ...
        for iteration in range(max_iterations):
            yield serialize_event(
                {"type": "status", "content": f"开始第 {iteration + 1} 次迭代"}
            )
            ...
            if subqueries_to_search:
                ...
                for item in search_results_list:
                    ...
                    for result in search_results:
                        ...
                        yield serialize_event(
                            {"type": "search_result_item", "result": memory[-1]}
                        )
            ...
            if memory:
                ...
                for item in memory[-20:]:
                    context_items.append(
                        f"  - {item['subquery']}: {item['summary'][:200]}..."
                    )
                memory_context = "\n".join(context_items)
...
"""
        ...
        try:
            ...
            yield serialize_event(
                {"type": "error", "content": f"报告生成出错: {str(e)}"}
            )
...
async def search_local_knowledge(
    query: str, kb_name: str, top_k: int = 5
) -> List[Dict]:
    """搜索本地知识库"""
    try:

...
def set_cached_search(query: str, results: List):
    ...
    expired_keys = [
        k for k, (_, t) in _search_cache.items() if current_time - t > SEARCH_CACHE_TTL
    ]
...
class ToolExecutor:
    ...
    async def execute(
        self, tool_name: str, params: Dict[str, Any], context: ReActContext
    ) -> Any:
        """
        执行指定工具
    ...
    async def execute_web_search(
        self, params: Dict[str, Any], context: ReActContext
    ) -> List[Dict]:
        """
        执行网络搜索
        ...
        if not query:
            logging.warning(
                "web_search called with empty query, using context query as fallback"
            )
            # 使用用户原始问题作为搜索关键词
            query = context.query
            if not query:
    ...
    def _websearch_sync(self, query: str, count: int = 5) -> List[Dict]:
        ...
        payload = json.dumps(
            {"query": query, "summary": True, "count": count, "page": 1}
        )
        headers = {
    ...
    async def execute_text2sql(
        ...
        try:
            ...
            return {"success": False, "error": "Text2SQL 服务尚未配置", "data": []}
        except Exception as e:
            logging.error(f"Text2SQL error: {e}")
            return {"success": False, "error": str(e), "data": []}

    # ========== Data Analyzer ==========
    async def execute_data_analyzer(
        self, params: Dict[str, Any], context: ReActContext
    ) -> Dict[str, Any]:
        """
        执行智能数据分析
    ...
    async def _simple_data_analysis(self, data: List) -> Dict[str, Any]:
        ...
        if numbers:
            ...
            insights.append(
                f"发现 {len(numbers)} 个数值数据，平均值 {statistics['numbers']['avg']}"
            )
        ...
        if percentages:
            ...
            insights.append(
                f"发现 {len(percentages)} 个百分比数据，范围 {statistics['percentages']['min']}% - {statistics['percentages']['max']}%"
            )
        ...
        if keywords:
            ...
            insights.append(
                f"高频关键词: {', '.join([f'{k}({v})' for k, v in top_keywords])}"
            )
    ...
    # ========== Chart Generator ==========
    async def execute_chart_generator(
        self, params: Dict[str, Any], context: ReActContext
    ) -> Dict[str, Any]:
        """
        生成图表配置
    ...
    def _simple_chart_config(
        self, data: Any, chart_type: str, title: str
    ) -> Dict[str, Any]:
        """简化版图表配置生成"""
        # 准备数据
        x_data = []
        ...
        base_config = {
            "type": chart_type,
            "title": title,
            "data": {"xAxis": x_data, "series": [{"name": "数值", "data": y_data}]},
            "options": {
                ...
                "series": [
                    {
                        "name": title,
                        "data": [
                            {"name": x, "value": y} for x, y in zip(x_data, y_data)
                        ],
                    }
                ]
            }
    ...
    async def execute_stock_query(
        ...
        try:
            ...
            return {"success": False, "error": "股票查询服务尚未配置", "data": None}
        except Exception as e:
            logging.error(f"Stock query error: {e}")
            return {"success": False, "error": str(e), "data": None}

    # ========== Bidding Search ==========
    async def execute_bidding_search(
        self, params: Dict[str, Any], context: ReActContext
    ) -> Dict[str, Any]:
        """
        执行招投标信息搜索
        ...
        try:
            ...
            result = await bidding_service.search_bids(
                keyword=keyword, category=category, region=region, page=page
            )

...
class ReActContext:
    ...
    def get_history_summary(self, max_items: int = 10) -> str:
        ...
        for step in self.steps[-max_items:]:
            ...
            if step.observation:
                result_str = (
                    str(step.observation.result)[:200]
...
"""
    ...
    async def _think(self, context: ReActContext) -> Thought:
        ...
        try:
            ...
            return Thought(
                reasoning=f"解析响应失败: {e}", should_finish=False, confidence=0.0
            )
        except Exception as e:
            logging.error(f"Error during thinking: {e}")
            return Thought(
                reasoning=f"思考过程出错: {e}", should_finish=True, confidence=0.0
            )
    ...
    def _validate_and_fix_params(
        ...
        if tool_name == ToolType.WEB_SEARCH.value:
            if not params.get("query"):
                ...
                extracted_query = self._extract_search_query_from_thought(
                    thought, context
                )
                if extracted_query:
            ...
            if not params.get("query"):
                params["query"] = context.query
                logging.warning(
                    f"Using context query for knowledge_search: {context.query}"
                )
    ...
    def _extract_search_query_from_thought(
        self, thought: str, context: ReActContext
    ) -> Optional[str]:
        """
        从思考内容中提取搜索关键词
    ...
    async def _execute_action(
        self, action: Action, context: ReActContext
    ) -> Observation:
        """
        执行动作 - 调用相应的工具
        ...
        try:
            ...
            return Observation(
                tool=action.tool, success=False, result=None, error=str(e)
            )
    ...
    async def run(
        ...
        while context.iteration < max_iterations:
            ...
            yield {
                "type": "status",
                "content": f"正在并行执行 {len(queries_to_execute)} 个搜索...",
            }
            ...
            if reflect_result["additional_queries"]:
                context.metadata["additional_queries"] = reflect_result[
                    "additional_queries"
                ]
                yield {
                    "type": "status",

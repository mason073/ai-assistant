...
```"""
    ...
    def _convert_flat_to_outline(self, flat_result: Dict) -> Dict:
        ...
        for i in range(1, 10):  # 最多支持9个章节
            ...
            section = {
                ...
                "requires_chart": i <= 2,
                "search_queries": [
                    flat_result.get(query_key, flat_result.get(title_key, ""))
                ],
            }
    ...
    async def _initial_planning(self, state: ResearchState) -> ResearchState:
        ...
        for attempt in range(max_retries + 1):
            ...
            # Debug: 记录原始响应
            self.logger.info(
                f"Raw LLM response length: {len(response)} (attempt {attempt + 1})"
            )
            ...
            if not result:
                self.logger.warning(
                    f"Attempt {attempt + 1}: JSON parsing failed completely"
                )
...
要求：outline必须包含5-8个章节，覆盖市场概况、企业竞争、技术趋势、政策环境、未来展望等方面。"""
        ...
        if not outline:
            self.logger.warning(
                f"No outline found! Full parsed result: {str(result)[:500]}"
            )

        # 更新状态
        state["key_entities"] = [
            e.get("name", "")
            for e in result.get("key_entities", [])
        ...
        for i, section in enumerate(outline):
            ...
            processed_section = {
                ...
                "search_queries": section.get(
                    "search_queries", [section.get("title", "")]
                ),
                "status": "pending",
            }
            ...
            if not isinstance(processed_section["search_queries"], list):
                processed_section["search_queries"] = [
                    str(processed_section["search_queries"])
                ]

...
```"""
    ...
    async def process(self, state: ResearchState) -> ResearchState:
        """处理入口"""
        self.logger.info("[CodeWizard] ========== process 开始 ==========")
        self.logger.info(
            f"[CodeWizard] 当前 phase: {state['phase']}, data_points: {len(state['data_points'])}, outline: {len(state['outline'])}"
        )
        ...
        if state["phase"] != ResearchPhase.ANALYZING.value:
            ...
            if len(state["data_points"]) >= 3:
                ...
                self.logger.info("[CodeWizard] 数据点足够，设置 phase 为 ANALYZING")
            else:
                self.logger.warning(
                    f"[CodeWizard] ⚠️ 数据点不足 ({len(state['data_points'])} < 3)，跳过分析"
                )
                return state
        ...
        # 执行数据分析
        self.logger.info("[CodeWizard] 开始执行 _analyze_data...")
        await self._analyze_data(state)
        self.logger.info(
            f"[CodeWizard] _analyze_data 完成，当前 charts 数量: {len(state['charts'])}"
        )

        # 生成图表
        self.logger.info("[CodeWizard] 开始执行 _generate_charts...")
        await self._generate_charts(state)
        self.logger.info(
            f"[CodeWizard] _generate_charts 完成，最终 charts 数量: {len(state['charts'])}"
        )

        self.logger.info("[CodeWizard] ========== process 结束 ==========")
        return state
    ...
    async def _analyze_data(self, state: ResearchState) -> None:
        ...
        prompt = self.ANALYSIS_PROMPT.format(
            query=state["query"], data_points="\n".join(data_summary)
        )

        self.logger.info(
            f"[CodeWizard] 调用LLM生成分析代码，数据点数量: {len(state['data_points'])}"
        )
        ...
        self.logger.info(
            f"[CodeWizard] LLM响应长度: {len(response)}, 前200字符: {response[:200]}"
        )
        ...
        self.logger.info(
            f"[CodeWizard] JSON解析结果: {type(result)}, keys: {result.keys() if isinstance(result, dict) else 'N/A'}"
        )
        ...
        if result and result.get("code"):
            ...
            self._save_debug_log(
                "3_code_raw", repr(code) if isinstance(code, str) else str(code)
            )
            self.logger.info(
                f"[CodeWizard] 原始code类型: {type(code)}, 长度: {len(str(code))}"
            )

            # 确保 code 是字符串类型
            if isinstance(code, list):
                code = "\n".join(str(c) for c in code)
                self.logger.info("[CodeWizard] code是list，已转换为字符串")
            elif not isinstance(code, str):
                code = str(code)
                self.logger.info("[CodeWizard] code不是字符串，已转换")
            ...
            self.logger.info(
                f"[CodeWizard] 清理后code行数: {cleaned_code.count(chr(10)) + 1}"
            )

            # ===== 详细日志: 语法检查 =====
            try:
                compile(cleaned_code, "<string>", "exec")
                self.logger.info("[CodeWizard] ✅ 语法检查通过")
                self._save_debug_log("6_syntax_check", "PASSED")
            except SyntaxError as e:
                self.logger.error(f"[CodeWizard] ❌ 语法检查失败: {e}")
                self._save_debug_log(
                    "6_syntax_check", f"FAILED: {e}\n\nCode:\n{cleaned_code}"
                )
            ...
            if (
                len(cleaned_code) < 50
                or '">\\' in cleaned_code
                or cleaned_code.count("\n") < 3
            ):
                self.logger.warning(
                    f"[CodeWizard] 检测到无效代码，跳过执行: {cleaned_code[:100]}..."
                )
                self._save_debug_log("7_validation", "INVALID: too short or bad format")
                return None
            ...
            self.logger.info("[CodeWizard] 开始执行代码...")

            # 执行代码（带自愈能力）
            execution_result = await self._execute_with_self_correction(code, state)
            ...
            if charts_generated:
                for i, chart_b64 in enumerate(charts_generated):
                    ...
                    self.logger.info(
                        f"[CodeWizard] Sent chart event: {chart_entry['title']}"
                    )

    async def _execute_with_self_correction(
        self, code: str, state: ResearchState, max_retries: int = 3
    ) -> Dict[str, Any]:
        """
        带自愈能力的代码执行
        ...
        while retries <= max_retries:
            ...
            if retries >= max_retries:
                self.logger.warning(
                    f"Code execution failed after {max_retries} retries: {error}"
                )
                return {
                    "success": False,
                    "error": error,
            ...
            self.logger.info(
                f"Attempting code self-correction (retry {retries + 1}/{max_retries})"
            )
            ...
            if (
                fixed_result
                and isinstance(fixed_result, dict)
                and fixed_result.get("fixed_code")
            ):
                ...
                self.logger.info(
                    f"Code fixed: {fixed_result.get('fix_description', 'N/A')}"
                )
    ...
    async def _generate_charts(self, state: ResearchState) -> None:
        ...
        if not chart_sections and state["outline"]:
            ...
            self.logger.info(
                "[CodeWizard] 没有 requires_chart 章节，使用前2个章节生成图表"
            )

        self.logger.info(
            f"[CodeWizard] 开始生成图表，需要图表的章节数: {len(chart_sections)}"
        )

        for i, section in enumerate(chart_sections[:2]):  # 最多生成2个图表
            self.logger.info(
                f"[CodeWizard] 处理章节 {i + 1}/{min(len(chart_sections), 2)}: '{section['title']}'"
            )

            # 收集相关数据
            section_data = self._get_section_data(state, section["id"])
            self.logger.info(
                f"[CodeWizard] 章节 '{section['title']}' 数据量: {len(section_data)}"
            )

            if not section_data:
                self.logger.warning(
                    f"[CodeWizard] ⚠️ 章节 '{section['title']}' 没有数据，跳过"
                )
                continue

            # 生成图表代码
            self.logger.info("[CodeWizard] 调用 LLM 生成图表代码...")
            chart_config = await self._generate_chart_code(
                topic=section["title"],
                data=section_data,
                chart_type="bar"
                if section.get("section_type") == "quantitative"
                else "line",
            ...
            if chart_config and chart_config.get("code"):
                ...
                self.logger.info(
                    f"[CodeWizard] ✅ 生成图表代码成功，长度: {len(chart_code)}"
                )
                ...
                if result.get("charts"):
                    ...
                    state["charts"].append(chart_entry)
                    self.logger.info(
                        f"[CodeWizard] 图表已添加到 state['charts']，当前总数: {len(state['charts'])}"
                    )
                    ...
                    self.logger.info(
                        f"[CodeWizard] ✅ 已发送 chart SSE 事件: {section['title']}"
                    )
                else:
                    self.logger.warning(
                        f"[CodeWizard] ⚠️ 图表执行失败或没有生成图表: success={result.get('success')}, error={result.get('error', 'N/A')[:100]}"
                    )
            else:
                self.logger.warning("[CodeWizard] ⚠️ LLM 没有返回有效的图表代码")

    def _get_section_data(self, state: ResearchState, section_id: str) -> List[Dict]:
        """获取章节相关数据"""
        related_facts = [
            f for f in state["facts"] if section_id in f.get("related_sections", [])
        ]
        related_data = []
    ...
    async def _generate_chart_code(
        self, topic: str, data: List[Dict], chart_type: str, title: str
    ) -> Optional[Dict]:
        """生成图表代码"""
        data_str = json.dumps(data, ensure_ascii=False, indent=2)

        prompt = self.CHART_PROMPT.format(
            topic=topic, data=data_str, chart_type=chart_type, title=title
        )

        response = await self.call_llm(
            system_prompt="你是数据可视化专家。", user_prompt=prompt, json_mode=True
        )
    ...
    def _clean_code(self, code: str) -> str:
        ...
        protected = re_module.sub(
            r"^\\([a-zA-Z_])", r"\1", protected, flags=re_module.MULTILINE
        )
        # 也处理不在行首但在空格后的情况
        protected = re_module.sub(
            r"(\s)\\([a-zA-Z_][a-zA-Z0-9_]*\s*=)", r"\1\2", protected
        )
    ...
    def _save_debug_info(
        self, raw_code: str, cleaned_code: str, error: Exception = None
    ):
        """保存调试信息到文件，方便排查问题"""
        import os
        from datetime import datetime
    ...
    async def _execute_code(self, code: str) -> Dict[str, Any]:
        ...
        if isinstance(code, list):
            code = "\n".join(str(c) for c in code)
            self.logger.info("[CodeWizard] 输入是list，已转换为字符串")
        elif not isinstance(code, str):
            code = str(code)
            self.logger.info("[CodeWizard] 输入非字符串，已转换")
        ...
        self.logger.info(
            f"[CodeWizard] 清理后代码行数: {code.count(chr(10)) + 1}, 长度: {len(code)}"
        )
        ...
        try:
            ...
            return {"success": False, "error": str(e), "output": "", "charts": []}
    ...
    def _execute_in_sandbox(self, code: str) -> Dict[str, Any]:
        ...
        """
        self.logger.info("[CodeWizard] _execute_in_sandbox 开始执行")
        ...
        self.logger.info(
            f"[CodeWizard] 沙箱环境准备完成，matplotlib backend: {matplotlib.get_backend()}"
        )
        ...
        try:
            ...
            self.logger.info("[CodeWizard] 开始 exec()...")
            self._save_debug_log(
                "sandbox_2_before_exec", f"即将执行代码，长度: {len(code)} 字符"
            )

            with redirect_stdout(stdout_capture), redirect_stderr(stderr_capture):
                exec(code, exec_globals)

            self.logger.info("[CodeWizard] exec() 完成")
            ...
            self.logger.info(
                f"[CodeWizard] exec() 输出: stdout={len(stdout_value)}字符, stderr={len(stderr_value)}字符"
            )
            self._save_debug_log(
                "sandbox_3_exec_output",
                f"stdout:\n{stdout_value}\n\nstderr:\n{stderr_value}",
            )

            if fig.get_axes():
                self.logger.info("[CodeWizard] 检测到图表，开始捕获...")
                # 重新应用字体到当前图表的所有文本元素
                chinese_fonts = [
                    "Heiti TC",
                    "STHeiti",
                ...
                plt.close(fig)
                self.logger.info(
                    f"[CodeWizard] 图表捕获成功，base64长度: {len(chart_b64)}"
                )
            else:
                self.logger.info("[CodeWizard] 未检测到图表")
            ...
            self._save_debug_log(
                "sandbox_4_result",
                f"success=True, charts={len(charts)}, output_len={len(stdout_value)}",
            )

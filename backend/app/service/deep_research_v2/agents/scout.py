...
```"""
    ...
    def __init__(
        ...
        if MILVUS_AVAILABLE:
            try:
                ...
                self.logger.info(
                    "Milvus service initialized for local knowledge base search"
                )
        ...
        self.add_message(
            ...
            "research_step",
            {
                ...
                "stats": {
                    ...
                    "sources_count": len(
                        set(f.get("source_url", "") for f in state.get("facts", []))
                    ),
                },
            },
        )
    ...
    async def _supplementary_research(self, state: ResearchState) -> ResearchState:
        ...
        self.logger.info(
            f"Starting supplementary research with {len(pending_queries)} queries"
        )
        ...
        for query in pending_queries[:5]:  # 最多处理5个补充查询
            self.add_message(
                state,
                "action",
                {"agent": self.name, "tool": "supplementary_search", "query": query},
            )
            ...
            "research_step",
            {
                ...
                "stats": {
                    ...
                    "sources_count": len(
                        set(
                            f.get("source_url", "")
                            for f in state.get("facts", [])[-new_facts_count:]
        ...
        try:
            ...
            for company_name, stock_code in found_companies[:2]:  # 最多查询2只股票
                ...
                if result.get("success"):
                    ...
                    self.logger.info(
                        f"获取股票数据成功: {data['name']} ¥{data['nowPri']}"
                    )
                else:
                    self.logger.warning(
                        f"获取股票数据失败: {stock_code} - {result.get('error')}"
                    )
    ...
    async def _analyze_supplementary_results(
        self, original_query: str, search_query: str, results: List[Dict]
    ) -> Optional[Dict]:
        """分析补充搜索结果"""
        results_text = []
        for r in results[:8]:
...
```"""
    ...
    def _emit_search_results_event(self, state: ResearchState) -> None:
        ...
        if search_results_for_ui:
            self.add_message(
                state, "search_results", {"results": search_results_for_ui}
            )
    ...
    async def _research_section(self, state: ResearchState, section: Dict) -> None:
        ...
        self.logger.info(
            f"Researching section: {section_title} (web={search_web}, local={search_local})"
        )
        ...
        analysis = await self._analyze_search_results(
            state["query"], section, all_results, hypotheses=state.get("hypotheses", [])
        )
        ...
        if analysis:
            ...
            if duplicate_facts > 0:
                self.logger.info(
                    f"Deduplicated {duplicate_facts} facts, added {added_facts}"
                )
    ...
    async def _execute_deep_search(
        ...
        for query in queries:
            ...
            analysis = await self._analyze_deep_search_results(
                state["query"], query, results, search_type, hypotheses
            )
            ...
            self.logger.info(
                f"Deep search ({search_type}, depth={depth}): +{added_facts} facts for query '{query[:30]}...'"
            )
    ...
    async def _analyze_deep_search_results(
        ...
        search_type_desc = (
            "追溯原始数据源" if search_type == "source_tracing" else "追踪相关线索"
        )
    ...
    async def _execute_local_search(self, query: str, top_k: int = 10) -> List[Dict]:
        ...
        try:
            ...
            results = self.milvus_service.search(
                collection_name="knowledge_base", query_vector=query_vector, top_k=top_k
            )
    ...
    async def _execute_search(self, query: str, count: int = 10) -> List[Dict]:
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.error(
                    f"Bocha API error: {response.status_code} - {response.text[:200]}"
                )
                return []
            ...
            if data.get("code") != 200:
                self.logger.error(
                    f"Bocha API returned error: {data.get('msg', 'Unknown error')}"
                )
                return []

            webpages = data.get("data", {}).get("webPages", {}).get("value", [])
            self.logger.info(
                f"Bocha search returned {len(webpages)} results for: {query[:30]}..."
            )
...
""")
    ...
    async def deep_read_url(self, url: str, title: str, query: str) -> Optional[Dict]:
        ...
        try:
            ...
            prompt = self.DEEP_READ_PROMPT.format(
                query=query, url=url, title=title, content=content
            )
    ...
    def _extract_text_from_html(
        self, html: str, url: str = "", max_length: int = 12000
    ) -> str:
        """
        从 HTML 中提取纯文本正文
        ...
        if TRAFILATURA_AVAILABLE:
            try:
                ...
                if text and len(text) > 200:
                    self.logger.debug(
                        f"Trafilatura extracted {len(text)} chars from {url}"
                    )
        ...
        if BS4_AVAILABLE:
            try:
                ...
                if text and len(text) > 200:
                    ...
                    self.logger.debug(
                        f"BeautifulSoup extracted {len(text)} chars from {url}"
                    )
                    return text[:max_length]
    ...
    def _update_knowledge_graph(
        self, state: ResearchState, entities: List[Dict]
    ) -> None:
        """更新知识图谱"""
        graph = state.get("knowledge_graph", {"nodes": [], "edges": []})
    ...
    def _update_hypothesis_status(
        self, state: ResearchState, evidence: List[Dict]
    ) -> None:
        """根据证据更新假设状态"""
        hypotheses = state.get("hypotheses", [])

...
```"""
    ...
    async def _write_section(self, state: ResearchState, section: Dict) -> None:
        ...
        for chart in state["charts"]:
            if chart.get("section_id") == section_id:
                charts_info.append(
                    f"- 图表: {chart.get('title')} (ID: {chart.get('id')})"
                )
    ...
    async def _synthesize_report(self, state: ResearchState) -> None:
        ...
        self.add_message(
            state,
            "thought",
            {"agent": self.name, "content": "正在整合各章节，生成完整研究报告..."},
        )
        ...
        for fact in state["facts"]:
            source_entry = (
                f"- {fact.get('source_name')} ({fact.get('source_url', 'N/A')})"
            )
        ...
        if result and result.get("full_report"):
            ...
            self.logger.info(
                f"[LeadWriter] ✅ 报告整合成功，长度: {len(state['final_report'])}"
            )
            ...
            self.logger.info(
                f"[LeadWriter] 使用备选报告，长度: {len(state['final_report'])}"
            )

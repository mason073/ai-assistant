...
class DeepResearchGraph:
    ...
    def __init__(
        ...
        self.data_analyst = DataAnalyst(
            self.llm_api_key, self.llm_base_url, config.agents.data_analyst.model
        )
        self.wizard = CodeWizard(
            self.llm_api_key, self.llm_base_url, config.agents.wizard.model
        )
        self.critic = CriticMaster(
            self.llm_api_key, self.llm_base_url, config.agents.critic.model
        )
        self.writer = LeadWriter(
            self.llm_api_key, self.llm_base_url, config.agents.writer.model
        )
    ...
    async def _run_simplified(
        ...
        try:
            ...
            for idx, ref in enumerate(final_raw_refs):
                fact = next(
                    (f for f in final_facts if f.get("source_url") == ref.get("url")),
                    None,
                )
                title = ref.get("source") or ref.get("marker") or ""
                if not title and fact:

...
export const knowledgeActions = {
  ...
  async updateKnowledgeBase(kbId: string, name?: string, description?: string) {
    const res = await knowledgeApi.updateKnowledgeBase(kbId, {
      name,
      description,
    })
    const index = knowledgeState.knowledgeBases.findIndex(
      (kb) => kb.id === kbId,
    )
    if (index !== -1) {
  ...
  async deleteDocument(kbId: string, docId: string) {
    ...
    if (knowledgeState.currentKnowledgeBase?.id === kbId) {
      knowledgeState.currentKnowledgeBase.documents =
        knowledgeState.currentKnowledgeBase.documents.filter(
          (doc) => doc.id !== docId,
        )
...
export type { KBDocument, KnowledgeBase, KnowledgeBaseWithDocuments }

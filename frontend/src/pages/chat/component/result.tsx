...
const renderObservationContent = (content: string) => {
  ...
  return lines.map((line, i) => {
    ...
    if (insightsMatch) {
      return (
        <div key={i} className={styles['obs-insights-title']}>
          洞察
        </div>
      )
    }
        ...
        return (
          ...
          >
            <div className={styles['react-step-header']}>
              {step.type !== 'plan' && (
                <span className={styles['react-step-badge']}>
                  Step {step.step}
                </span>
              )}
              <span className={styles['react-step-icon']}>
                {getStepIcon(step.type)}
              </span>
              <span className={styles['react-step-label']}>
              ...
              {isClickable && (
                <span className={styles['react-step-detail-hint']}>
                  点击查看详情 →
                </span>
              )}
            </div>
            <div className={styles['react-step-content']}>
              {step.content != null &&
                (typeof step.content === 'string' && step.content ? (
                  step.type === 'plan' ? (
                    <Markdown
                      className={styles['react-step-markdown']}
                      value={step.content}
                    />
  ...
  return (
    <div className={styles['chat-message-result']}>
      <Section title="智能回答" icon={IconThink} defaultOpen>
        {/* ReAct 智能推理过程 */}
        {item.reactSteps?.length ? (
          <ReAct过程 item={item} onStepClick={onStepClick} />
        ) : null}

        {/* 传统思考过程 */}
        {item.thinks && !item.reactSteps?.length ? (
          <思考过程 item={item} />
        ) : null}

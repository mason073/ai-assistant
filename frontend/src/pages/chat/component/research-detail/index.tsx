...
export default function ResearchDetail({
  ...
  console.log(
    `[ResearchDetail] 渲染，data=${data ? 'exists' : 'null'}, steps=${steps.length}`,
  )
  if (data) {
    console.log(
      `[ResearchDetail] data 详情: searchResults=${data.searchResults?.length || 0}, charts=${data.charts?.length || 0}, hasGraph=${!!data.knowledgeGraph}, hasReport=${!!data.streamingReport}`,
    )
  }
  ...
  const reportCount =
    (data?.sections?.length || 0) + (data?.streamingReport ? 1 : 0)
  ...
  return (
    <div className={styles.panel}>
        ...
        <div className={styles.stepper}>
          {steps.map((step, index) => (
            <div
              ...
              onClick={() =>
                step.status === 'completed' && onStepClick?.(step.id)
              }
            >
      ...
      <div className={styles.content}>
        ...
        {activeTab === 'graph' && (
          <KnowledgeGraph data={data?.knowledgeGraph} />
        )}
        {activeTab === 'charts' && <Visualization charts={data?.charts} />}
        {activeTab === 'report' && (
          <ProcessReport
            content={data?.streamingReport}

...
export default function Visualization({ charts }: VisualizationProps) {
  ...
  if (charts?.length) {
    charts.forEach((c, i) => {
      console.log(
        `[Visualization] 图表 ${i + 1}: id=${c.id}, title=${c.title}, type=${c.type}, has_echarts=${!!c.echarts_option}, has_image=${!!c.image_base64}`,
      )
    })
  }
      ...
      {charts.map((chart) => (
        <div key={chart.id} className={styles.card}>
          <div className={styles.cardHeader}>
            ...
            {chart.subtitle && (
              <p className={styles.cardSubtitle}>{chart.subtitle}</p>
            )}
          </div>
